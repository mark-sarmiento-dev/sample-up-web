<template>
  ...
  >
    <VCard class="custom-modal-shadow rounded-lg pa-5">
      ...
      <!-- Subtext (no ellipsis, wraps properly) -->
...
watch(
  ...
  (val) => {
    isOpen.value = val
    if (val) {
      isDeleting.value = false // reset every time modal opens
    }
  }
)
...
// parent must call this after API finishes
...
<style scoped>
.custom-modal-shadow {
  /* Same embossed modal shadow */
  box-shadow: 0 12px 28px rgba(0, 0, 0, 0.18) !important;
  border-radius: 12px;
}

/* Make sure subheader text wraps cleanly */
.description-text {
